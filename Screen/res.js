...
const getS = async() => {
  ...
  const data =await getUserData();
    ...
    fetch("http://192.168.1.15:3001/reservation/getByS/"+data.data.station._id
)
    .then((res) => res.json())
    .then((resJSON) => {
      console.warn(resJSON)
...
navigation.navigate('Dashboard')
  ...
  <Text style={{fontWeight:'bold' , marginStart:10}}> {item.etat ? (
                            <Text style={{color:'green'}}> Reservation Confirmer</Text>
                  ...
                  <View style={{
                        ...
                        <AntDesign name="checkcircle" size={20} color="#22780F" onPress={() => {
                          ...
                          <AntDesign name="eyeo" size={26} color="black" style={{marginStart:30 }} onPress={() => {
